...
export type SiteOneLookup = PublicOriginLookup;
...
function validateInput(input: SiteOneInput): PublicOrigin {
  ...
  try {
    ...
    if (error instanceof PublicOriginError) throw new SiteOneInputError(error.message);
    throw error;
  }
...
export async function executeSiteOne(
  ...
  try {
    resolvedAddresses = await resolvePublicOrigin(origin, dependencies?.lookup);
  } catch (error) {
    if (error instanceof PublicOriginError) throw new SiteOneInputError(error.message);
    throw error;
  }
